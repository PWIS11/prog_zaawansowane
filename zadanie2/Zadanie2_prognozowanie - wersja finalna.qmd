...
Zastosowane modele:
**A. Dla obiektu badań** (energy_consumption): Model Holt-Winters zastosowano model potrójnego wygładzania wykładniczego , znany jako metoda Holta-Wintersa.
...
```{python}
#| colab: {base_uri: https://localhost:8080/, height: 823}
...
# A. Prognoza dla OBIEKTU  - Zużycie Energii
forecast_energy = make_forecast(df, 'energy_consumption')

# B. Prognoza dla OTOCZENIA - Cena Rynkowa
...
# === DODATKOWY WYKRES: ZMIENNA OTOCZENIA===
plt.figure(figsize=(14, 7))

# Historia Ceny
plt.plot(df.index[-60:], df['market_price'].iloc[-60:], label='Historia Cen', color='gray')

# Prognoza Ceny
plt.plot(forecast_price.index, forecast_price, label='Prognoza Ceny (Otoczenie)', color='orange', linestyle='--', linewidth=2)

plt.title('Prognoza zmiennej otoczenia: Cena Rynkowa (Market Price)', fontsize=14)
plt.legend()
plt.grid(True, alpha=0.3)
plt.ylabel('Cena (EUR/MWh)')
plt.show()
print("=== Prognozowane wartości (pierwsze 5 miesięcy) ===")
print(forecast_price.head())
```

**Interpretacja prognozy zmiennej otoczenia:**

1. **Stabilizacja po kryzysie:** Wykres wyraźnie pokazuje, że historyczny szok cenowy z 2022 roku (szara linia, pik powyżej 400 EUR/MWh) wygasł. Prognoza (linia pomarańczowa) nie zakłada powrotu do tak ekstremalnych wartości w scenariuszu bazowym.
2. Model przewiduje, że w latach 2026–2028 ceny ustabilizują się w przedziale ok. 80–110 EUR/MWh. Jest to poziom wyższy niż przed 2021 rokiem, ale znacznie niższy niż w szczycie kryzysu.
3. **Sezonowość:** Linia prognozy nie jest płaska, lecz faluje. Oznacza to, że model poprawnie wyłapał sezonowość rynku energii – ceny zmieniają się cyklicznie w zależności od pór roku (wzrosty w okresach większego zapotrzebowania lub mniejszej generacji OZE).
...
**Cel symulacji:**
Celem tego etapu jest wyznaczenie obszaru niepewnosci dla niemieckiego rynku energii na lata 2024–2026. Wizualizacja w formie wykresu wachlarzowego pozwoli określić nie tylko najbardziej prawdopodobny przebieg zużycia energii, ale także graniczne wartości ryzyka  oraz potencjału wzrostowego).
...
Przeprowadzona symulacja pozwoliła na wyznaczenie stoka niepewności dla niemieckiego rynku energii na lata 2024–2026. Analiza ujawniła fundamentalną rozbieżność między inercyjnym trendem spadkowym a potencjałem transformacyjnym.
